...
def update_disc_on_inn(
    ...
    for _ in range(args.num_disc_updates):
        encoding_t = inn.encode(x_t, stochastic=args.stochastic)
        encoding_c = inn.encode(x_c, stochastic=args.stochastic)
        # disc_input_c: Tensor
        # if args.train_on_recon:
        #     disc_input_c = inn.decode(encoding_c).detach()  # just reconstruct
        disc_input_t: Tensor
        if args.train_on_recon:
            disc_input_t = inn.decode(encoding_t).detach()  # just reconstruct

        disc_loss = x_c.new_zeros(())
        for invariance in invariances:
            if not args.train_on_recon:
                disc_input_t = get_disc_input(
                    args, inn, encoding_t, invariant_to=invariance, random=False
                )
                disc_input_t = disc_input_t.detach()
            disc_input_c = get_disc_input(
                args, inn, encoding_c, invariant_to=invariance, random=False
            )
...
def update_inn(
    ...
    enc_t, sum_ldj_t, ae_enc_t = inn.encode_with_ae_enc(
    ...
    enc_c, sum_ldj_c, ae_enc_c = inn.encode_with_ae_enc(
