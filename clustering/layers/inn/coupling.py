...
class AffineCouplingLayer(CouplingLayer):
    ...
    def logdetjac(self, scale: Tensor) -> Tensor:
        return sum_except_batch(torch.log(scale), keepdim=True)

    def _scale_and_shift_fn(self, inputs: Tensor) -> Tuple[Tensor, Tensor]:
...
class AdditiveCouplingLayer(CouplingLayer):
    def __init__(
        ...
        self.net_t: BottleneckConvBlock = BottleneckConvBlock(
            in_channels=self.d,
    ...
    def _shift_fn(self, inputs: Tensor) -> Tensor:
...
class IntegerDiscreteFlow(AdditiveCouplingLayer):
    def __init__(self, in_channels: int, hidden_channels: int, depth: int = 3) -> None:
    ...
    def _get_shift_param(self, inputs: Tensor) -> Tensor:
...
class MaskedCouplingLayer(Bijector):
    ...
    @staticmethod
    def logdetjac(scale: Tensor) -> Tensor:
...
def sample_mask(dim: int, mask_type: str, swap: bool) -> Tensor:
...
def build_net(input_dim: int, hidden_dims: Sequence[int], activation: str = "relu") -> nn.Module:
