...
from configs.enums import DiscriminatorMethod
from src.arch.aggregation import BatchAggregator
...
from src.models.autoencoder import AutoEncoder, SplitEncoding
...
class SupportMatching(AdvSemiSupervisedAlg):
    ...
    def _build_discriminator(self, encoder: AutoEncoder, *, dm: DataModule) -> Discriminator:
        ...
        disc_fn = Fcn(
            hidden_dim=self.alg_cfg.disc_hidden_dim,
            num_hidden=self.alg_cfg.disc_num_hidden,
        ...
        if self.alg_cfg.aggregator_type is not None:
            final_proj = (
                Fcn(
                    hidden_dim=self.alg_cfg.agg_hidden_dim,
                    num_hidden=self.alg_cfg.agg_num_hidden,
                )
                if self.alg_cfg.agg_hidden_dim
                else None
            )
            aggregator_cls: type[BatchAggregator] = self.alg_cfg.aggregator_type.value
            ...
            backbone, _ = disc_fn(input_dim=encoder.encoding_size.zy, target_dim=1)
    ...
    def _encoder_loss(
        ...
        with torch.cuda.amp.autocast(enabled=self.misc_cfg.use_amp):  # type: ignore
            ...
            if not warmup:
                ...
                disc_input_dep = None
                if self.alg_cfg.double_adv_loss:
